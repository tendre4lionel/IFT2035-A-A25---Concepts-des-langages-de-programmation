...
and parse_ident (input : char list) : (string * char list) option =
  ...
  (
    ...
    if (not (is_letter (List.nth input 0))) then None else (
      let rec read (position : int): int = if (is_letter (List.nth input position)) then read (position+1) else position in
      let split = read 0 in
      let input = Array.of_list input in
      let sub_word = Array.to_list(Array.sub input 0 split) in 
      let word = implode sub_word in
      let rest = Array.to_list(Array.sub input (split+1) (Array.length input)) in 
      (word,rest)
    ) 
  )
